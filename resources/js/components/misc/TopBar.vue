<template>
  <nav class="bg-white border-gray-200 dark:bg-gray-900 sticky top-0 insex-x-0 w-full z-50">
    <div
      ...
      <div class="flex">
        <ul
          class="flex items-center md:p-2  rounded-lg   md:space-x-8 md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
          <li>
            <!-- <a href="#" class="block py-2 pl-3 pr-4 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white md:dark:text-blue-500" aria-current="page">Home</a> -->
            <Menu as="div" class="relative">
              <MenuButton class="-m-1.5 flex items-center p-1.5 space-x-5">
                <span class="sr-only">Open user menu</span>
                <img class="h-8 w-8 rounded-full bg-gray-50 "
                  src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80"
                  alt="" />
                <span class="hidden lg:flex lg:items-center dark:text-white">{{ user_full_name
                }}
                </span>
              </MenuButton>
              <transition enter-active-class="transition ease-out duration-100"
                ...
                <MenuItems
                  ...
                  <a :href="item.href"
                    :class="[active ? 'bg-gray-50' : '', 'block px-3 py-1 text-sm leading-6 text-gray-900 capitalize']">{{
                      item.name }}</a>
                  </MenuItem>
                </MenuItems>
              </transition>

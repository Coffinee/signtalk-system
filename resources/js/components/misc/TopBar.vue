<template>
  <nav class="bg-white border-gray-200 dark:bg-gray-900 sticky top-0 insex-x-0 w-full z-50">
    <div class="relative flex flex-wrap items-center justify-around mx-auto p-4 border-b border-gray-200 dark:border-gray-700">
      ...
      <div :show="open == true" :class="[!open ? 'hidden w-full md:block md:w-auto space-x-5' : 'absolute top-[55px] left-0 right-0 z-10']">
          ...
          <li>
            <Menu as="div" class="relative">
                <MenuButton class="-m-1.5 flex items-center p-1.5">
                    ...
                    <img class="h-8 w-8 rounded-full bg-gray-50" src="../../../assets/images/Vedasto.png" alt="Profile" />
                    <span class="hidden lg:flex lg:items-center">
                    <span class="ml-4 text-sm font-semibold leading-6 text-gray-900" aria-hidden="true">Vedaddy</span>
                ...
                <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                    ...
                    <MenuItem v-for="item in userNavigation" :key="item.name" v-slot="{ active }">
                        <a :href="item.href" :class="[active ? 'bg-gray-50' : '', 'block px-3 py-1 text-sm leading-6 text-gray-900 capitalize']">{{ item.name }}</a>
                    </MenuItem>
                    </MenuItems>
                </transition>
                </Menu>

...
<template>
  ...
  <div>
            ...
            <DialogPanel class="relative mr-16 flex w-full max-w-xs flex-1">
              ...
              <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-gray-900 px-6 pb-2 ring-1 ring-white/10">
                <div class="flex h-16 shrink-0 items-center">
                  <p class="text-white text-xl font-bold">Sign<span>Talk</span></p>
                </div>
                      ...
                      <ul role="list" class="-mx-2 space-y-1">
                        <li v-for="item in navigation" :key="item.name">
                          <router-link :to="item.href"
                            :class="[useRoute().path == item.href ? 'bg-gray-800 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800', 'group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold']">
                            <component :is="item.icon"
                              :class="[useRoute().path == item.current ? 'text-white' : 'text-indigo-200 group-hover:text-white', 'h-6 w-6 shrink-0']"
                              aria-hidden="true" />
                            {{ item.name }}
                          </router-link>
                        </li>
    ...
    <div class="lg:pl-72">
        ...
        <div class="flex flex-1 gap-x-4 self-stretch lg:gap-x-6">
          <form class="relative flex flex-1" action="#" method="GET">
            ...
            <input id="search-field"
              class="block h-full w-full border-0 py-0 pl-8 pr-0 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm bg-white"
            ...
            <Menu as="div" class="relative">
              <MenuButton class="-m-1.5 flex items-center p-1.5">
                ...
                <span class="hidden lg:flex lg:items-center">
                  <span class="ml-4 text-sm font-semibold leading-6 text-gray-900" aria-hidden="true">{{ user_fname }} {{
                    user_lname }}</span>
              ...
              <transition enter-active-class="transition ease-out duration-100"
                ...
                <MenuItems
                  ...
                  <router-link to="#"
                    class="block px-3 py-1 text-sm leading-6 text-gray-900 text-center">Settings</router-link>
                  <router-link to="#" @click.prevent="logout"
                    class="block px-3 py-1 text-sm leading-6 text-gray-900 text-center">Sign Out</router-link>
                </MenuItems>
              </transition>
            </Menu>
          </div>
      ...
      <main class="h-screen py-10 bg-white">

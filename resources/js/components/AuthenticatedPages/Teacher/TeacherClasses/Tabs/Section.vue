<template>
        ...
        <div class="flex justify-between items-center">
            ...
            <div class="flex gap-[10px]">
                <button @click="(approvalListOpen = !approvalListOpen)"
                    class="hover:text-white hover:bg-indigo-500 rounded-md border border-indigo-500 text-indigo-500 text-base w-[150px] h-auto py-1 px-2">For Approval
                </button>
        ...
        <div class="overflow-auto">
            <table class="w-full text-sm text-left text-gray-500 py-[10px]">
                <thead class="text-xs text-gray-700 uppercase bg-gray-200">
                    <tr>
                        <th v-for="label in officialTable" :key="label.label" scope="col" class="px-6 py-3 whitespace-nowrap">
                            {{ label.label }}
                        </th>
                    </tr>
                </thead>
                <tbody v-for="item in officialStudents" :key="item.id">
                    <tr v-if="officialStudents && item.classCode == this.classCode" class="bg-white border-b">
                        ...
                        <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                            <button @click.prevent="(studentReportOpen = !studentReportOpen), getStudentQuizResults(item)" class="text-blue-400 underline">View Report</button>
                        </td>
                    </tr>
                </tbody>
            </table>
    ...
    <Modal :show="studentReportOpen" @close="viewStudentReportToggle" :title="'Student Report'" :heightModal="'h-[500px]'" :widthModal="'w-[1000px]'">
        <TabGroup :selectedIndex="selectedTab" @change="changeTab">
            <TabList class=" flex justify-center gap-[5px] text-sm font-medium text-center text-black dark:text-gray-400 dark:border-gray-700 w-full mb-[10px]">
                <Tab
                    v-for="tab in studentReportTab"
                    as="template"
                    :key="tab"
                    v-slot="{ selected }"
                    >
                        ...
                        <div class="overflow-auto">
                            <table class="w-full text-sm text-left text-gray-500 py-[10px]">
                                <thead class="text-xs text-gray-700 uppercase bg-gray-200 text-center">
                                    <tr>
                                        <th v-for="label in studentQuizRecord" :key="label.label" scope="col" class="px-6 py-3 whitespace-nowrap">
                                            {{ label.label }}
                                        </th>
                                    </tr>
                                </thead>
        ...
        <div class="w-full h-full">
            <div class="overflow-auto">
                <table class="w-full text-sm text-left text-gray-500 py-[10px]">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-200 text-center">
                        <tr>
                            <th v-for="label in pendingTable" :key="label.label" scope="col" class="px-6 py-3 whitespace-nowrap">
                                {{ label.label }}
                            </th>
                        </tr>
                    </thead>
                    <tbody v-for="item in pendingStudents" :key="item.id">
                        <tr v-if="pendingStudents && item.classCode == this.classCode" class="bg-white border-b text-center">
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap capitalize">
                                {{ item.status }}
                            </th>
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
...
export default {
    ...
    methods: {
        ...
        getStudentQuizResults(studentId) {
            ...
            axios.get('api/getstudentquiz/' + studentId)
                .then(response => {
                    this.quizRecordStudent = response.data;
                })
                .catch(error => {
                    console.error(error);
                });
